Given('que a base de dados já está configurada') do
  # Configuração inicial da base de dados
  @database = Database.new
  expect(@database.configured?).to be true
end

Given('os dados atuais do SIGAA estão disponíveis') do
...
When('eu solicito a atualização da base de dados') do
...
Then('a base de dados deve ser atualizada com os dados atuais do SIGAA') do
  # Verifica se a base de dados foi atualizada corretamente
  expect(@database.updated_with?(@sigaa_data)).to be true
end

Then('devo receber uma confirmação de que a atualização foi concluída com sucesso') do
...
Given('os dados atuais do SIGAA não estão disponíveis') do
...
Then('a atualização deve falhar') do
  # Verifica que a atualização não foi realizada
  expect(@update_result).to be_nil
end

Then('devo receber uma mensagem de erro informando que o SIGAA está indisponível') do
